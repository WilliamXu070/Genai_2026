...
(async () => {
  ...
  const artifactsDir = "C:\\Users\\William\\Desktop\\Projects\\Genai_2026\\db\\langflow_agentic_runs\\run_4608_1773511595039";
  ...
  await page.locator("#terminal-output").first().waitFor({ state: 'visible', timeout: 10000 });
  stateStore[""] = await page.locator("#terminal-output").first().innerText({ timeout: 10000 });
  ...
  await page.waitForTimeout(10000);
  await page.waitForFunction(({ selector, before }) => { const el = document.querySelector(selector); if (!el) return false; const current = (el.innerText || el.textContent || '').trim(); return current !== before; }, { selector: "#terminal-output", before: (stateStore[""] || '') }, { timeout: 10000 });
  await page.screenshot({ path: path.join(artifactsDir, `step_${Date.now()}.png`), fullPage: true });
  } catch (error) {
    status = 'fail';
