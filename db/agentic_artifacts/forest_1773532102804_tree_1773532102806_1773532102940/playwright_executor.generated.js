...
async function run({ baseUrl, artifactsDir }) {
  ...
  await expect(page.locator("#active-label").first()).toBeVisible();
  await page.waitForTimeout(500);
  ...
  await page.waitForTimeout(500);
  await expect(page.locator("#flight-path").first()).toBeVisible();
  await page.waitForTimeout(500);
  await page.locator("text=Black Box").first().click();
  await page.waitForTimeout(500);
  await expect(page.locator("#black-box").first()).toBeVisible();
  await page.waitForTimeout(500);
  await page.goto("text=Hangar", { waitUntil: 'domcontentloaded' });
  await page.waitForTimeout(700);
  await expect(page.locator("#hangar").first()).toBeVisible();
  await page.waitForTimeout(500);
  await page.locator("text=View hangar notes").first().click();
  await page.waitForTimeout(500);
  await page.locator("text=Open the hangar").first().click();
  await page.waitForTimeout(500);
  } finally {
    await context.close();
    await browser.close();
