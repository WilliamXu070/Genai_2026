...
async function run({ baseUrl, artifactsDir }) {
  ...
  await expect(page.locator("text=Plant Growth Cycle").first()).toBeVisible();
  ...
  await page.screenshot({ path: path.join(artifactsDir, 'step_1773550485568.png'), fullPage: true });
  await page.waitForTimeout(500);
  } finally {
    await context.close();
    await browser.close();
